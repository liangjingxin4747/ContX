...
class VQModelwithSPRef(pl.LightningModule):
    def __init__(self,
        ...
        self.fuse_layer = nn.Sequential(
            Conv2d(
            self.out_channels + self.SPk,   # ddconfig.z_channels + self.SPk * self.out_channels
            self.out_channels,
            kernel_size=3,
            stride=1,
            padding=1
        ...
        # self.recon_decoder = Decoder(**ddconfig)
    ...
    def get_shape_prior(self, pred_context_logits):   # Tensor(bs,87,h,w)
        ...
        classes = list(torch.argmax(reconstruction, dim=-1))   # Tensor(bs)
        max_pred = pred_context_logits[list(np.arange(bs)), classes, :, :].view(bs,1,h,w)   # Tensor(bs,1,h,w)
        
        nn_latent_vectors = self.recon_net.encode(max_pred)   # Tensor(bs,8,h/8,w/8)
        # classes = torch.tensor([self.cls_gray_dict[cls.item()] for cls in classes]).to(self.device)
    ...
    def forward(self, input):  # Tensor(bs,87,256,256)
        ...
        shape_prior = self.get_shape_prior(pred_context_logits)   # Tensor(bs,SPk,h,w)
        # shape_prior = F.avg_pool2d(shape_prior, 16)   # Tensor(bs,c*16,h/16,w/16)
        # pred_context_logits_ = self.decoder(self.fuse_layer(cat([quant, shape_prior], dim=1)))
        pred_context_logits_ = self.fuse_layer(cat([pred_context_logits, shape_prior], dim=1))

        return pred_context_logits_, diff
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        self.mask_recon_inference(dis_x)

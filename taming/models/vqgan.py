...
class VQModel(pl.LightningModule):
    def __init__(self,
        ...
        ddconfig['z_channels'] = ddconfig['z_channels'] + self.noise_dim
    ...
    def encode(self, x):   # Tensor(bs,87,256,256)
        ...
        quant = self.quantize(h)   # Tensor(bs,256,16,16)
        return quant
        return quant, emb_loss, info # revised
    ...
    def forward(self, input):   # Tensor(bs,87,256,256)
        ...
        # dec, noise = self.decode(quant)   # Tensor(bs,87,256,256)
        # if self.noise_dim != 0:
        #     dec1, noise1 = self.decode(quant)   # Tensor(bs,87,256,256)
        #     return [dec, dec1, noise, noise1], diff
        # else:
        #     return dec, diff
        quant = self.encode(input)   # Tensor(bs,256,16,16)
...
class VAEModel(pl.LightningModule):
    def __init__(self,
        ...
        self.decoder_input_chw = (256, 8, 8)
        self.decoder_projection = nn.Linear(self.latent_dim, 8*8*256)
        self.mean_linear = nn.Linear(8*8*256, self.latent_dim)
        self.var_linear = nn.Linear(8*8*256, self.latent_dim)
        if ckpt_path is not None:
            self.init_from_ckpt(ckpt_path, ignore_keys=ignore_keys)
        if colorize_nlabels is not None:
            assert type(colorize_nlabels)==int
    ...
    def encode(self, x):   # Tensor(bs,87,256,256)
        h = self.encoder(x)   # Tensor(bs,256,16,16)
        h = F.relu(h)   # Tensor(bs,256,16,16)
        return h

    def decode(self, quant):   # Tensor(bs,256)
        quant = self.decoder_projection(quant)   # Tensor(bs,256*8*8)
        quant = torch.reshape(quant, (-1, *self.decoder_input_chw))   # Tensor(bs,256,8,8)
        if self.noise_dim != 0:
            noise = quant.new_empty(quant.shape[0], self.noise_dim, 1).normal_().to(self.device)   # Tensor(bs,16,1)
            noise = noise.repeat(1, 1, quant.shape[2]*quant.shape[3]).view(-1, self.noise_dim, quant.shape[2], quant.shape[3])   # Tensor(bs,16,16,16)
            quant = torch.cat([quant, noise], dim=1)   # Tensor(bs,256,16,16)
    ...
    def decode_code(self, code_b):
        quant_b = self.decoder_projection(code_b)
        quant_b = torch.reshape(quant_b, (-1, *self.decoder_input_chw))
        dec = self.decode(quant_b)
        return dec

    def forward(self, input):   # Tensor(bs,87,128,128)
        encoded = self.encode(input)   # Tensor(bs,256,8,8)
        
        encoded = torch.flatten(encoded, 1)   # Tensor(bs,256*8*8)
        mean = self.mean_linear(encoded)   # Tensor(bs,256)
        logvar = self.var_linear(encoded)   # Tensor(bs,256)
        eps = torch.randn_like(logvar)   # Tensor(bs,256)
        std = torch.exp(logvar / 2)   # Tensor(bs,256)
        z = eps * std + mean   # Tensor(bs,256)
        kl_loss = torch.mean(-0.5 * torch.sum(1 + logvar - mean**2 - torch.exp(logvar), 1), 0)
        
        dec, noise = self.decode(z)   # Tensor(bs,87,256,256)
        if self.noise_dim != 0:
            dec1, noise1 = self.decode(z)   # Tensor(bs,87,256,256)
            return [dec, dec1, noise, noise1], kl_loss
        else:
            return dec, kl_loss
    ...
    def configure_optimizers(self):
        ...
        opt_ae = torch.optim.Adam(list(self.encoder.parameters())+
                                  list(self.decoder.parameters())+
                                  list(self.decoder_projection.parameters())+
                                  list(self.mean_linear.parameters())+
                                  list(self.var_linear.parameters()),
                                  lr=lr, betas=(0.5, 0.9))
        opt_disc = torch.optim.Adam(self.loss.discriminator.parameters(),
                                    lr=lr, betas=(0.5, 0.9))
        return [opt_ae, opt_disc], []
...
class VQModelwithSPRef(pl.LightningModule):
    ...
    def get_shape_prior(self, pred_context_logits):   # Tensor(bs,87,h,w)
        ...
        # classes = torch.tensor([156 for i in range(len(classes_ori))]).to(self.device)
        z_q, shape_prior = self.recon_net.nearest_decode(nn_latent_vectors, classes, k=self.SPk)   # Tensor(bs,SPk,h,w)

        sp = torch.zeros((bs, c, h, w)).to(self.device)   # Tensor(bs,87,h,w)
        for i, cls in enumerate(classes_ori):
            sp[i, cls, :, :] = shape_prior[i, 0, :, :]
    ...
    def forward(self, input):  # Tensor(bs,87,256,256)
        ...
        shape_prior = self.get_shape_prior(pred_context_logits)   # Tensor(bs,87,h,w)
        
        pred_context_logits_ = pred_context_logits.add(shape_prior)   # Tensor(bs,87,h,w)
